<template>
  <div class="container mx-auto p-4 max-w-4xl pb-24">
    <!-- Header -->
    <div class="flex justify-between items-start mb-8 bg-base-200/50 p-6 rounded-2xl shadow-sm border border-base-200">
      <div>
        <h1 class="text-3xl font-bold text-base-content mb-2">{{ title || (loading ? 'Loading...' : 'Details') }}</h1>
      </div>
      <div class="flex gap-3">
        <button @click="toggleBookmark" class="btn btn-circle bg-base-100 shadow-sm border-base-300 hover:border-error hover:bg-error/10 hover:text-error transition-colors" :class="{'text-error border-error bg-error/10': isBookmarked}" title="Toggle Bookmark">
        ...
        <button @click="openPresenter" class="btn btn-primary gap-2 shadow-md" :disabled="lines.length === 0">
    ...
    <div v-else class="space-y-6">
      ...
      >
        ...
        <div v-if="line.description" class="bg-primary/10 text-base-content p-6 rounded-2xl mb-6 shadow-inner" :dir="isRtl ? 'rtl' : 'ltr'">
        ...
        <!-- Line Content Block -->
        <div v-if="line.arabic || line.translation || line.transliteration" class="card bg-base-100 shadow-sm border border-base-200 hover:shadow-md transition-shadow duration-300">
          <div class="card-body p-8">
            <p v-if="line.arabic" class="text-4xl text-right mb-6 leading-loose font-arabic text-base-content" :style="{ fontFamily: fontConfig.arabic, letterSpacing: '0' }" dir="rtl">
              {{ line.arabic }}
            </p>
            <div class="divider opacity-30" v-if="line.arabic && (line.translation || line.transliteration)"></div>
            <p v-if="line.translation" class="text-xl font-medium mb-3 font-translation text-primary" :style="{ fontFamily: fontConfig.translation }" :dir="isRtl ? 'rtl' : 'ltr'">
              {{ line.translation }}
            </p>
            <p v-if="line.transliteration" class="text-lg text-base-content/60 font-transliteration italic" :style="{ fontFamily: fontConfig.transliteration }">
    ...
    <!-- Empty State -->
    <div v-if="!loading && lines.length === 0" class="text-center py-20 bg-base-200/30 rounded-2xl border border-base-200 border-dashed">
      <p class="text-lg text-base-content/60">No content found.</p>
    </div>
  </div>
</template>
